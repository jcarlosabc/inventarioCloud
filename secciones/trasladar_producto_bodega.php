...
if ($_POST) {   
    ...
    if ($lista_producto_buscado) {
        echo " SI hay un producto con ese codigo... aqui va upd";
        echo "<br>";
        // Actualizando la cantidad en el stock del LOCAL que le pasamos de bodega
        $sql = "UPDATE producto SET producto_fecha_ingreso = ?, producto_stock_total = producto_stock_total + ? WHERE producto_codigo = ? AND link= ?";
        $sentencia_envio = $conexion->prepare($sql);
        $params = array($fechaActual,$cantidad_enviada,$producto_codigo, $link_empresa);
     ...
     $sql = "INSERT INTO producto (producto_codigo, producto_fecha_creacion, producto_fecha_garantia,producto_nombre, producto_stock_total,
        producto_precio_compra,producto_precio_venta,producto_marca,producto_modelo, categoria_id,proveedor_id, link, responsable )         
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
